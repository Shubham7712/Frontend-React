...
export default function Employee() {
    ...
    const deleteData = (id) => {
		return MySwal.fire({
			...
			if (result.value) {
				axios.get('http://localhost:5000/Admin/User/delete/' + id).then(
					(res) => {
...
const 	handlesearch = () => {
	...
	if (Search) {
		api.post('Admin/User/getsearchdata', postData).then((res) => {
...

        {
			...
			cell: (row) => {
				return (
                    ...
                    <>
                        <DragSwitch checked={row.IsActive === true ? true : false} />
                    </>
                )
                }
		},
